            ...
            <div class="overflow-x-auto rounded-lg border border-gray-200 dark:border-white/10">
                <table class="w-full text-sm">
                    ...
                    @forelse ($overview['records'] as $technician)
                        <tr wire:key="technician-salary-{{ $technician->id }}" class="even:bg-gray-50 dark:even:bg-white/5">
                            ...
                            <td class="px-3 py-2 text-right">
                                @if ($technician->salary_active && $technician->salary_type)
                                    <x-filament::button size="xs" wire:click="openSalary({{ $technician->id }})">{{ __('employees.salary.open_review') }}</x-filament::button>
                                @endif
                                <x-filament::button size="xs" color="gray" wire:click="openHistory({{ $technician->id }})">{{ __('employees.salary.history') }}</x-filament::button>
                            </td>
                        </tr>
                    @empty
